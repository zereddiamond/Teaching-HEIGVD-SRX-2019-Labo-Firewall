...
| :---:             | :---:                  | :---:| :------: | :------: | :----: |
|     192.168.100.0/24   |  anywhere (eth0)  |   UDP/TCP   |  |   53       |  ACCEPT      |
|    anywhere (eth0)    |   192.168.100.0/24 |   UDP/TCP   |  53 |        |  ACCEPT      |
|     192.168.100.0/24   |  anywhere (eth0)  | ICMP  request   |          |          |  ACCEPT      |
|     anywhere (eth0)   |   192.168.100.0/24 | ICMP  reply   |          |          |  ACCEPT      |
|     192.168.100.0/24   |  192.168.200.0/24 | ICMP  request   |          |          |  ACCEPT      |
|     192.168.200.0/24   |  192.168.100.0/24 | ICMP  reply   |          |          |  ACCEPT      |
|     192.168.200.0/24   |  192.168.100.0/24  | ICMP request   |          |          |  ACCEPT      |
|     192.168.100.0/24   |  192.168.200.0/24  | ICMP  reply  |          |          |  ACCEPT      |
|     192.168.100.0/24   |  anywhere (eth0)   |  TCP    |          |   80       | ACCEPT       |
|   anywhere (eth0) |  192.168.100.0/24   |  TCP    |     80     |          | ACCEPT       |
| 192.168.100.0/24   |  anywhere (eth0)   |  TCP    |          |   8080       | ACCEPT       |
|  anywhere (eth0)   |   192.168.100.0/24  |  TCP    |    8080      |       | ACCEPT       |
|192.168.100.0/24   |  anywhere (eth0)   |  TCP    |          |   443      | ACCEPT       |
| anywhere (eth0)  |  192.168.100.0/24   |  TCP    |     443     |       | ACCEPT       |
|192.168.100.0/24   |  192.168.200.3/24   |  TCP    |          |   80      | ACCEPT       |
|192.168.200.3/24   |  192.168.100.0/24   |  TCP    |      80    |        | ACCEPT       |
|anywhere (eth0)  |  192.168.200.3/24   |  TCP    |          |   80      | ACCEPT       |
|192.168.200.3/24 |  anywhere (eth0)    |  TCP    |      80    |        | ACCEPT       |
...
![1ère tentative du ping](figures/config_base_ping_1er.png)
...
![2ème tentative de ping](figures/config_base_ping2.png)
...
![Tentative de ping vers le WAN depuis le client](figures/config_base_ping_lan_wan.png)

![Tentative de ping vers le WAN depuis la DMZ](figures/config_base_ping_dmz_wan3.png)
---
...
iptables -A FORWARD -s 192.168.200.0/24 -d 192.168.100.0/24 -p icmp --icmp-type 0 -m conntrack --ctstate ESTABLISHED -j ACCEPT

iptables -A FORWARD -s 192.168.100.0/24 -o eth0 -p icmp --icmp-type 8 -j ACCEPT
iptables -A FORWARD -i eth0 -d 192.168.100.0/24 -p icmp --icmp-type 0 -m conntrack --ctstate ESTABLISHED -j ACCEPT

iptables -A FORWARD -s 192.168.200.0/24 -d 192.168.100.0/24 -p icmp --icmp-type 8 -j ACCEPT
iptables -A FORWARD -s 192.168.100.0/24 -d 192.168.200.0/24 -p icmp --icmp-type 0 -m conntrack --ctstate ESTABLISHED -j ACCEPT
...
| :---                 | :---: | :---                         |
| Interface DMZ du FW  |   OK    |     L'interface fait partie de la DMZ et les règles du firewall authorise les PING vers la DMZ                          |
| Interface LAN du FW  |   OK    |   La route par défaut est configuré pour être le firewall                            |
| Client LAN           |   OK   |      C'est comme si nous pingons la machine directement, comme un ping localhost                            |
| Serveur WAN          |   OK    |     Le firewall authorise le traffic ICMP entre le LAN et le WAN                         |


| De Server\_in\_DMZ à | OK/KO | Commentaires et explications |
| :---                 | :---: | :---                         |
| Interface DMZ du FW  |  OK     |     La route par défaut pour la DMZ est configuré pour être le firewall                          |
| Interface LAN du FW  |  OK     |   Application de la règle d'authoriser les requêtes ICMP du DMZ au LAN                           |
| Serveur DMZ          |  OK     |       Auto-ping                        |
| Serveur WAN          |  KO    |     Pas de règles firewall pour les requêtes ICMP de la DMZ au WAN                          |
...
![Tentative de ping avec résolution DNS](figures/ping_dns_lan_wan_avant_config.png)
...
iptables -A FORWARD -d 192.168.100.0/24 -i eth0 -p udp --sport 53 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT

iptables -A FORWARD -s 192.168.100.0/24 -o eth0 -p tcp --dport 53 -j ACCEPT
iptables -A FORWARD -d 192.168.100.0/24 -i eth0 -p tcp --sport 53 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT
```
...
**LIVRABLE : capture d'écran de votre ping.**
![Tentative de ping avec résolution DNS](figures/ping_dns_lan_wan_apres_config.png)
...
iptables -A FORWARD -d 192.168.100.0/24 -i eth0 -p tcp --sport 80 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT

iptables -A FORWARD -s 192.168.100.0/24 -o eth0 -p tcp --dport 8080 -j ACCEPT
iptables -A FORWARD -d 192.168.100.0/24 -i eth0 -p tcp --sport 8080 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT

iptables -A FORWARD -s 192.168.100.0/24 -o eth0 -p tcp --dport 443 -j ACCEPT
iptables -A FORWARD -d 192.168.100.0/24 -i eth0 -p tcp --sport 443 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT
```
...
iptables -A FORWARD -s 192.168.100.0/24 -d 192.168.200.3/24 -p tcp --dport 80 -j ACCEPT
iptables -A FORWARD -s 192.168.200.3/24 -d 192.168.100.0/24 -p tcp --sport 80 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT

iptables -A FORWARD -i eth0 -d 192.168.200.3/24 -p tcp --dport 80 -j ACCEPT
iptables -A FORWARD -o eth0 -s 192.168.200.3/24 -p tcp --sport 80 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT
```
---
...
![wget vers le serveur Web de la DMZ](figures/config_lan_serveur_web_wget.png)
...
iptables -A FORWARD -s 192.168.100.3/24 -d 192.168.200.3/24 -p tcp --dport 22 -j ACCEPT
iptables -A FORWARD -s 192.168.200.3/24 -d 192.168.100.3/24 -p tcp --sport 22 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT

iptables -A INPUT -s 192.168.100.3/24 -p tcp --dport 22 -j ACCEPT
iptables -A OUTPUT -d 192.168.100.3/24 -p tcp --sport 22 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT
```
...
![connexion ssh vers le serveur Web depuis le LAN](figures/config_ssh_lan_dmz.png)

![connexion ssh vers le firewall depuis le LAN](figures/config_ssh_lan_firewall.png)
---
...
**LIVRABLE : Votre réponse ici...**
Permet une gestion à distance en ouvrant une connexion sécurisée vers une ressource comme un serveur afin de pouvoir l'administrer.
...
**LIVRABLE : Votre réponse ici...**
Il faut prêter une attention particulière lorsqu'on applique une règle qui ouvre un port acccessible depuis l'extérieur et aussi la complexité du mot de passe SSH.
...
![liste des règles du firewall](figures/list_all_rules_firewall.png)
